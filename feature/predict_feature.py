    ...
    def process_data(self, df, add_properties=True, add_pubchem=True,
        ...
        if self.is_training and log_transform_mic and self.mic_column in result_df.columns:
            ...
            if not valid_mic_mask.all():
                ...
                print(
                    f"Removed {invalid_count} entries with non-positive {self.mic_column} values before log transformation")
...
def predictioniction_features(input_file, output_file, smiles_column='Smiles'):
...
def file_prediction_features(input_file, output_file, smiles_column='Smiles'):
    processor = FeatureProcessor(
        smiles_column=smiles_column,
        is_training=False
    )
